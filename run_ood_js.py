...
from data import load_datasets, load_dataset_with_kl
...
parser.add_argument(
    "--path-js", default='./data/imageGPT_Evaluation_Results_NLL.p', type=Path, help="Path to image directory"
)
...
def get_loader_in_dist_nll(path, clusters_path, sample_flag=False, device=None, batch_size=16):
...
def get_loader_in_dist_js(path, clusters_path, sample_flag=False, device=None, batch_size=16):
    train, test = load_dataset_with_kl(path=path, clusters_path=clusters_path, sample_flag=sample_flag, device=device)

    dataset = TensorDataset(test[0] / 255., test[1])
...
in_dist_loader = get_loader_in_dist_nll(args.path, args.path_to_clusters, device=device, batch_size=args.batch_size)
in_dist_loader_js = get_loader_in_dist_js(args.path_js, args.path_to_clusters, device=device, batch_size=args.batch_size)
...
results['js_in'] = eval_model(model_reverse, in_dist_loader_js, desc='JS in')
results['js_out'] = eval_model(model_reverse, out_dist_loader, desc='JS out')

with open((out_path / "js_loss" / f"{args.ood_data_name}.json").as_posix(), "w") as f:
    json.dump(results, f)
